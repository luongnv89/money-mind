...
export const getDemoTransactions = (): Transaction[] => {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        // Fixed Expenses
        addTx(baseDay + 1, 'LANDLORD RENT PAYMENT', -1400.00, 'Rent');
        addTx(baseDay + 3, 'VERIZON WIRELESS', -85.00, 'Bills');
        addTx(baseDay + 5, 'STATE FARM INSURANCE', -62.50, 'Insurance');
        addTx(baseDay + 28, 'COMCAST INTERNET', -79.99, 'Bills');
        
        // Variable Monthly Utilities
        addTx(baseDay + 15, 'CITY UTILITIES WATER', -45.00 - (Math.random() * 15), 'Utilities');
        addTx(baseDay + 20, 'PG&E ELECTRIC', -50.00 - (Math.random() * 30), 'Utilities');
        ...
        // Occasional Shopping
        addTx(baseDay + 10 + Math.floor(Math.random() * 5), 'AMZN Mktp US*1A2B3C', -25.00 - (Math.random() * 50), 'Shopping');
    }
    ...
    for (let i = 0; i < 7; i++) {
        const day = (i * 14) + 2;
        if (day > 90) break;
        addTx(day, 'GUSTO PAYROLL 123456', 2850.00, 'Income');
    }

    // --- Weekly Groceries ---
    const groceryStores = ['TRADER JOES #123', 'WHOLE FOODS MARKET', 'SAFEWAY 2342', 'COSTCO WHOLESALE', 'LOCAL FARMERS MARKET'];
    for (let i = 0; i < 13; i++) {
    ...
    for (let i = 0; i < 90; i += 10) {
         ...
         addTx(i + Math.floor(Math.random() * 3), 'CHEVRON 00234', -45.00 - (Math.random() * 15), 'Gas');
         
         // Rideshare occasionally
         if (Math.random() > 0.5) {
            addTx(i + 5, 'UBER TRIP 23424', -15.00 - (Math.random() * 20), 'Travel');
         }
    }

    // --- Daily/Random Spending (Coffee, Dining) ---
    const coffeeShops = ['STARBUCKS STORE 12342', 'BLUE BOTTLE COFFEE', 'PEETS COFFEE', 'LOCAL CAFE'];
    const diningPlaces = ['CHIPOTLE ONLINE', 'UBER EATS', 'DOORDASH*MCDONALDS', 'LOCAL DINER 55', 'SWEETGREEN', 'SHAKE SHACK'];
