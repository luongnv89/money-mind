    ...
    try {
      ...
      if (processed.length > 0) {
        setAnalysisStats({
          ...
          mediumConfidence: processed.filter((t) => t.confidence >= 0.5 && t.confidence < 0.8)
            .length,
  ...
  if (transactions.length === 0) {
    return (
      <div className="text-center py-20 animate-in fade-in duration-500">
        ...
        {isAddModalOpen && (
          <AddTransactionModal onClose={() => setIsAddModalOpen(false)} onSave={handleManualAdd} />
        )}
      </div>
    );
  }
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="bg-yellow-50 rounded-lg p-3 border border-yellow-100">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <div className="text-xs text-yellow-700 font-medium uppercase tracking-wide">
                  Medium Confidence
                </div>
              </div>
              <div className="text-2xl font-bold text-yellow-700">
                {analysisStats.mediumConfidence}
              </div>
