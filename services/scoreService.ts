...
export const calculateFinancialScore = (allTransactions: Transaction[]): FinancialScore => {
  ...
  if (expenses.length < 5) {
    return {
      ...
      grade: '-',
      breakdown: [],
      tips: ['Add more transactions to generate a score.'],
      hasEnoughData: false,
    };
  }
  ...
  if (sortedTx.length === 0) {
    return {
      ...
      grade: '-',
      breakdown: [],
      tips: ['No valid transaction dates found.'],
      hasEnoughData: false,
    };
  }
  ...
  if (tips.length === 0) {
    if (score > 90)
      tips.push("You're crushing it! Consider increasing your investment contributions.");
