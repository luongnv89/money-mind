                ...
                <div>
                    <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2">
                            <div className={cn("p-1.5 rounded-md", colorClass.replace('text-', 'bg-').replace('600', '100').replace('700', '100'))}>
                        ...
                        {average > 0 && (
                            <div className={cn(
                                ...
                                {Math.abs(diffPercent)}% {diffPercent > 0 ? (type === 'expense' ? 'Over' : 'Up') : (type === 'expense' ? 'Under' : 'Down')}
                            </div>
                        )}
                    </div>
  ...
  const budgetMetrics = useMemo(() => {
    ...
    const calculateTotals = (txs: Transaction[]) => {
        return txs.reduce((acc, t) => {
            ...
            else if (t.category === TransactionCategory.Save || t.category === TransactionCategory.Invest) acc.savings += amt;
            return acc;
      ...
      <div className="space-y-3">
          <div className="flex items-center justify-between px-1">
            <h3 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
                ...
                Monthly Performance <span className="text-gray-400 font-normal">({budgetMetrics.monthLabel})</span>
            </h3>
          </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <Card className="border-gray-200 flex flex-col min-h-[300px]">
          <CardHeader className="pb-0 pt-4 px-4">
              <CardTitle className="text-sm font-medium text-gray-700">Spending Mix (Current View)</CardTitle>
          </CardHeader>
          <CardContent className="flex-1 p-2">
              {pieData.length > 0 ? (
                  ...
                  <div className="h-full flex items-center justify-center text-gray-400 text-sm">No data in current view</div>
              )}
          </CardContent>
      </Card>
      ...
      <Card className="border-gray-200 flex flex-col min-h-[300px]">
          ...
          <CardContent className="flex-1 p-2 pl-0">
               {trendData.length > 0 ? (
                  ...
                  <div className="h-full flex items-center justify-center text-gray-400 text-sm">Not enough history</div>
               )}
          </CardContent>
      </Card>
    </div>
