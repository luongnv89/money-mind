...
- **Tailwind ships from CDN with its config inlined in `index.html`.** No `tailwind.config.js` or `postcss.config.js` exists; the `tailwindcss`/`postcss`/`autoprefixer` devDeps are vestigial. Edit custom tokens (`accent`, `accent-light`, `secondary`) in that inline `<script>`.
- **A single `any` fails lint** — `no-explicit-any` is warn-level and lint runs at zero warnings.
- **Keep Vitest on 3.x.** Vitest 2 refuses Vite 6 as a peer and nests its own Vite 5, which makes `tsc` fail on `vite.config.ts`.
